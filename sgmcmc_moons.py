...
def accuracy_cnn(params, X, y):
    target_class = y
    predicted_class = model.apply(params, X)>0.5
    return predicted_class.ravel() == target_class
...
def sgld_sampler(key,log_post, grad_log_post, num_samples,
    ...
    for i in range(num_samples):
        ...
        test_data_loader = get_dataloader(test_data[0],test_data[1],test_data[0].shape[0],key_test)
...
samples,loss,accuracy=sgld_sampler(key_state_init,log_post,
                                               grad_log_post,10000,1e-5,
                                               params,train_data,
                                               test_data,batch_size)

batch_evaluate=jax.vmap(accuracy_cnn,in_axes=(None,0,0))
results=batch_evaluate(params,Xs_test,Ys_test)

batch_init=jax.vmap(model.init,in_axes=(0,None))
batch_fit=jax.vmap(sgld_sampler,in_axes=(0,None,None,None,None,0,0,None))